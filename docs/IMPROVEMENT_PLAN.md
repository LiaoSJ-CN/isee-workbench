...
| 下一批：批 4a 参数 schema 后端 | ⏳ **下次会话从这里开始**（按已重排顺序：4a → 2a → 2b → 3a → 3b → 4b → 6b → 1.5 → 7 → 8 → 9 → 10） |

**下一会话怎么接：**

1. 打开本文件 → 看「当前进度」表
2. 跑 `make test-fast && make lint && make typecheck && make build` 确认基线没漂
3. 读 plan 文件 `~/.claude/plans/cozy-brewing-falcon.md` 中「批 4a」章节
4. 建 TaskCreate 覆盖批 4a 子项（`ReportParameter` model + Pydantic discriminated union + 持久化 + `GET /reports/{id}/parameters`），开始干
