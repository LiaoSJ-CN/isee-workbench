...
def test_conftest_cleanup_prunes_ds2_and_r2_pollution(db, admin_user):
    """Regression guard for the 2026-08-30 incident.

    ``test_pin_version_from_other_report_404`` (above) creates a *second*
    report + DataSource with the ``ds2_<uuid>`` / ``r2_<uuid>`` name
    pattern. If that test crashes / gets SIGKILLed before the autouse
    ``_cleanup_leaked_data_source_rows`` fixture in ``conftest.py`` runs
    its post-yield prune, those rows leak into ``backend/app.db`` (the
    dev DB). Conftest's prune previously matched ``ds\\_%`` / ``r\\_%``
    but NOT ``ds2\\_%`` / ``r2\\_%`` — the missing pattern caused the
    five ``ds2_*`` + ``r2_*`` ghost rows the user manually swept on
    2026-08-30.

    This test re-creates the pollution rows, runs the same LIKE clauses
    conftest uses, and asserts both rows are gone while the live
    ``sqlite_demo`` ds and demo reports survive.
    """
    from sqlalchemy import text

    # Mimic the leak: insert a sibling DS + Report + ReportVersion the way
    # ``test_pin_version_from_other_report_404`` does.
    leaked_ds = DataSource(
        name=_unique("ds2"),
        db_type="sqlite",
        database=":memory:",
        owner_user_id=admin_user.id,
    ...
    db.add(leaked_ds)
    db.commit()
    db.refresh(leaked_ds)

    leaked_report = Report(
        name=_unique("r2"),
        data_source_id=leaked_ds.id,
        owner_user_id=admin_user.id,
    )
    db.add(leaked_report)
    db.commit()
    db.refresh(leaked_report)

    # Insert a ReportVersion so the report_versions child-prune clause
    # has something to sweep (matches the production leak shape).
    leaked_version = ReportVersion(
        report_id=leaked_report.id,
        version_number=1,
        name=leaked_report.name,
        data_source_id=leaked_ds.id,
        created_by=admin_user.id,
    )
    db.add(leaked_version)
    db.commit()

    # Replicate the EXACT LIKE clauses from conftest's
    # ``_cleanup_leaked_data_source_rows`` so any future conftest drift
    # breaks this test loudly. Keep these in sync with conftest.py.
    db.execute(
        text(
    ...
    # Both leaked rows are gone ...
    assert (
        db.query(DataSource).filter(DataSource.name.like("ds2_%")).count() == 0
    )
    assert db.query(Report).filter(Report.name.like("r2_%")).count() == 0
    # ... but the live demo data is intact.
    demo_ds = (
        db.query(DataSource).filter(DataSource.name == "sqlite_demo").first()
    )
    assert demo_ds is not None, "live demo DataSource must survive the prune"
    demo_reports = (
        db.query(Report)
        .filter(
            Report.name.in_(
                ["财务经营月报", "应收账款分析", "供应商付款与应付分析"]
            )
        )
        .count()
    )
    assert demo_reports == 3, "3 demo reports must survive the prune"
