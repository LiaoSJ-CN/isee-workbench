...
# ---------------------------------------------------------------------------
# Dashboard HTML renderer (批 14.4 — moved out of routers/dashboard.py)
# ---------------------------------------------------------------------------
...
def render_dashboard_html(
    ...
    ``{html, items_rendered, items_failed}`` so callers (preview
    endpoint + dispatcher) can surface partial-success banners when
    one item blows up.

    Moved here from :mod:`app.routers.dashboard` in 批 14.4 so the
    subscription dispatcher can reuse the exact same render pipeline
    without importing the router module (which would re-introduce a
    service→router dependency the codebase otherwise avoids).
    """
    items_rendered = 0
    items_failed = 0
    chunks: list[str] = []
