...
def upgrade() -> None:
    ...
    op.create_index(
        op.f('ix_reports_org_id'), 'reports', ['org_id'], unique=False
    )
    ...
    op.create_table(
        ...
        sa.UniqueConstraint(
            'report_id', 'user_id', name='uq_report_access_report_user'
        ),
    )
    op.create_index(
        op.f('ix_report_access_id'), 'report_access', ['id'], unique=False
    )
    op.create_index(
...
def downgrade() -> None:
    """Downgrade schema."""
    op.drop_index(
        op.f('ix_report_access_user_id'), table_name='report_access'
    )
    op.drop_index(
        op.f('ix_report_access_report_id'), table_name='report_access'
    )
    op.drop_index(
        op.f('ix_report_access_id'), table_name='report_access'
    )
    ...
    with op.batch_alter_table('reports') as batch_op:
        batch_op.drop_constraint(
            'fk_reports_owner_user_id_users', type_='foreignkey'
        )
